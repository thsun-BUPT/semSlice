...
log_dir = f"./Records/noslice/SS/fitSNR/{script_name}_{start_time}/"
...
if __name__ == '__main__':
    ...
    average_ssimilarity_per_slice = [sum(row[j] for row in total_ssimilarity) / epoch for j in range(slices_num)]
    average_S_SE_per_slice = [sum(row[j] for row in total_S_SE) / epoch for j in range(slices_num)]

    print("\n {}轮后,各切片的平均similarity是:{}".format(epoch,average_ssimilarity_per_slice))
    print("\n {}轮后,各切片的平均S-SE是:{}".format(epoch,average_S_SE_per_slice))
    

    results_dir = f"./Records/noslice/SS/fitSNR/{script_name}_{start_time}/"
