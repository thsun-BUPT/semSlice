...
def select_slice_after(sem_NSSAI,type):
    slice_index=1
    if sem_NSSAI >= 95:
    
        slice_index = 0
    elif  85 <= sem_NSSAI < 95 and type=="HF" :

        slice_index = 0
    elif  75 <= sem_NSSAI < 85 and type=="RT" :
...
def tasks_after(args):
    ...
    for i in range(tasks_num):
        ...
        path= args.__dict__.get('task_{}_vocab'.format(i+1))
        sem_NSSAI = count_duplicate_keys(path) 
        slice_idx = select_slice_after(sem_NSSAI,task_type[i]) 
        slices_list[slice_idx].append(task_list[i][0]) 
        task_list[i].append(slice_idx+1)
...
def encoder_choose(args,task):
    path= args.__dict__.get('task_{}_vocab'.format(task)) 
    sem_NSSAI = count_duplicate_keys(path) 
    if sem_NSSAI >= 95:
...
tasks_list_after,slices_list_after,task_data_set_after,deepsc_models_after = tasks_after(args)
def main_compute_after(X): 


    
    K = k_symbol
    ssimilarity = None
    ssimilarity_list=[[] for _ in range(slices_num_emergency)]
    ssimilarity_mean_list=[]
    s_se = None
    S_SE_list=[[] for _ in range(slices_num_emergency)]
    S_SE_mean_list=[]
    args = parser.parse_args()
    ...
    for i in range(slices_num_emergency):
    
        P=X[i]
        B=X[i+2]
    

        for j in range(len(slices_list_after[i])):
            Task_idx = slices_list_after[i][j]-1  
            tasks_count += 1

            print("\n ----STRAT--\n 切片号：{}，任务号：{}, 进度 {}/{}".format(i,slices_list_after[i][j],tasks_count,tasks_num))

            Encoder_idx = encoder_choose(args,slices_list_after[i][j])  
            ...
            tasks_results[Task_idx] = {
                "Task_id": Task_idx, 
                "Slice_id": slices_list_after[i][j], 
                "Similarity": sim_score,
                "Delay": delay_transmit,
                "S-SE": s_se_socre
            }
