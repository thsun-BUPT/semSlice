...
log_dir = f'./Records/noslice/emergency/{script_name}_{start_time}/'
...
def tasks_after(args,task_list): 
    """
    任务分配
    """
    ...
    task_list=[[] for _ in range(tasks_num)] 
    slices_list=[[] for _ in range(slices_num_emergency)] 
    task_data_set={}
    deepsc_models={}
    ...
    while True:
        encoder_idxs = [random.choice([1,2,3]) for _ in range(tasks_num)]
        if 1 in encoder_idxs and 2 in encoder_idxs and 3 in encoder_idxs:
            break
    ...
    for i in range(tasks_num):
        if old_task_list[i][1] == 2: 
            old_task_list[i][1] = random.choice([1, 3]) 
    ...
    for i in range(tasks_num):
        if old_task_list[i][1] == 1:
            slices_list[0].append(i+1)
        elif old_task_list[i][1] == 3:
            slices_list[1].append(i+1)
...
def performance_after(args, SNR, Encoder_idx,Task_idx):

    similarity = Similarity(args.bert_config_path, args.bert_checkpoint_path, args.bert_dict_path)



    net = deepsc_models_after[Encoder_idx]
...
def compute_performance_after(P,B,K,Encoder_idx,Task_idx):
    ...
    sim_score, _  = performance_after(args, SNR, Encoder_idx,Task_idx)
...
if __name__ == '__main__':
    np.random.seed(4)
    random.seed(4)
    total_tasks_list=[[] for _ in range(tasks_num)] 
    ssimilarity = None
    total_ssimilarity=[]
    total_ssimilarity_after=[]
    avg_ssimilarity=[]
    avg_ssimilarity_after=[]
    s_se = None
    total_S_SE=[]
    total_S_SE_after=[]
    avg_S_SE=[]
    ...
    B_total = 2 
    k = 10
    total_tasks_list=[[] for _ in range(epoch)] 
    
    data_results = [[{} for _ in range(tasks_num)] for _ in range(epoch)]


    data_results_after = [[{} for _ in range(tasks_num)] for _ in range(epoch)]
    ...
    results_dir = f"./Records/noslice/emergency/{script_name}_{start_time}/"
    ...
    for t in range(1):
        ...
        for i in range(tasks_num):
            ...
            P=0.00834970495229781
            B=1.0290313831067206
            ...
            '''CHANGE'''
            if tasks_list[i][1] == 1:
                sim_score,total_delay,delay_transmit = compute_performance(P_1,B_1,k,Encoder_idx,Task_idx)
            else:
                sim_score,total_delay,delay_transmit = compute_performance(P,B,k,Encoder_idx,Task_idx)
            ssimilarity = sim_score 
            if 0<= i <=2:
    ...
    if not os.path.exists(results_dir):
        os.makedirs(results_dir)  
    results_filename = os.path.join(results_dir, f"{script_name}_{start_time}_badchannel.json")
    with open(results_filename, 'w', encoding='utf-8') as f:
        json.dump(data_results, f, ensure_ascii=False, indent=4)
    print(f"\n Data has been saved to {results_filename}")
    ...
    for t in range(epoch):
        ...
        ssimilarity_list_after=[]
        ssimilarity_mean_list_after=[0.0] * slices_num

        S_SE_list_after=[]
        S_SE_mean_list_after=[0.0] * slices_num

        print('--------------Epoch {}/{}  Beginning! ------------- \n'.format(t+1,epoch))

        tasks_list_after,slices_list_after,task_data_set_after,deepsc_models_after = tasks_after(args,total_tasks_list[t])
        ...
        for i in range(tasks_num):
            ...
            Task_idx = i 
            Encoder_idx = tasks_list_after[i][1] - 1  
            print("切片号：{}，任务号：{}, 进度 {}/{} \n".format(Encoder_idx+1,Task_idx+1,i+1,tasks_num))
            
            '''CHANGE'''
            sim_score,total_delay,delay_transmit = compute_performance_after(P,B,k,Encoder_idx,Task_idx)
            ssimilarity = sim_score 
            if 0<= i <=2:
                if ssimilarity < args.sim_threshold: 
                    ssimilarity=0
            ...
            ssimilarity_list_after.append(ssimilarity)
            ssimilarity_mean_list_after[Encoder_idx] += ssimilarity  

            S_SE_list_after.append(s_se)
            S_SE_mean_list_after[Encoder_idx] += s_se
            data_results[t][i] = {
                "Task_id": Task_idx, 
                "Slice_id": Encoder_idx, 
                "Similarity": sim_score,
        ...
        print("\n 当前的SNR为: ",compute_SNR(P,B))
        print("\n 各epoch各切片的similarity是:{}".format(total_ssimilarity_after))
        print("\n ssimilarity_list is(各切片各任务的similarity为):",ssimilarity_list_after)
        print("\n ssimilarity_mean_list is(每个切片的similarity为):",ssimilarity_mean_list_after)
        print("\n S_SE_list is(各切片各任务的S-SE为):",S_SE_list_after)
        print("\n S_SE_mean_list is(每个切片的S-SE为):",S_SE_mean_list_after)
        print('\n --------------Epoch {}/{}  End! -------------'.format(t+1,epoch))

    print("\n -------实验结束------")
    print("\n 当前的SNR为: ",compute_SNR(P,B))
    print("\n 各epoch各切片的ssimilarity是:{}".format(total_ssimilarity_after))
    print("\n 各epoch各切片的S-SE是:{}".format(total_S_SE_after))


    results_dir = f"./Records/noslice/emergency/{script_name}_{start_time}/"
    if not os.path.exists(results_dir):
        os.makedirs(results_dir)  
    results_filename = os.path.join(results_dir, f"{script_name}_{start_time}_emergency.json")
